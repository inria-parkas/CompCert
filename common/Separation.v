...
Lemma storev_rule:
  ...
  spec (Val.load_result chunk v) ->
  exists m',
  Mem.storev chunk m (Vptr b (Int.repr ofs)) v = Some m' /\ m' |= contains' p chunk b ofs spec ** P.
Proof.
...
Lemma storev_rule':
  ...
  m |= contains' p chunk b ofs spec1 ** P ->
  exists m',
  Mem.storev chunk m (Vptr b (Int.repr ofs)) v = Some m' /\ m' |= hasvalue' p chunk b ofs (Val.load_result chunk v) ** P.
Proof.
